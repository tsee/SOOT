...
$myc->Divide(1,2);
my $pad1 = $myc->cd(1)->as('TPad'); # FIXME the dreaded enum bug strikes back;
$pad1->SetLogy();
$pad1->SetGrid();
...
my $f1 = $fgamma->DrawCopy()->as('TF1'); # FIXME the dreaded enum bug strikes back
$f1->SetMinimum(1e-5);
$f1->SetLineColor(kRed);
$fgamma->SetParameters(1, 0, 1);
my $f2 = $fgamma->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
$f2->SetLineColor(kGreen);
$fgamma->SetParameters(2, 0, 1);
my $f3 = $fgamma->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
$f3->SetLineColor(kBlue);
$fgamma->SetParameters(5, 0, 1);
my $f4 = $fgamma->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
...
# TMath::LogNormal
my $pad2 = $myc->cd(2)->as('TPad'); # FIXME the dreaded enum bug strikes back
$pad2->SetLogy();
$pad2->SetGrid();
my $flog = TF1->new("flog", "TMath::LogNormal(x, [0], [1], [2])", 0, 5);
$flog->SetParameters(0.5, 0, 1);
my $g1 = $flog->DrawCopy()->as('TF1'); # FIXME the dreaded enum bug strikes back
$g1->SetLineColor(kRed);
$flog->SetParameters(1, 0, 1);
my $g2 = $flog->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
$g2->SetLineColor(kGreen);
$flog->SetParameters(2, 0, 1);
my $g3 = $flog->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
$g3->SetLineColor(kBlue);
$flog->SetParameters(5, 0, 1);
my $g4 = $flog->DrawCopy("same")->as('TF1'); # FIXME the dreaded enum bug strikes back;
