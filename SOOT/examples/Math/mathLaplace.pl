...
$c1->Divide(1, 2);
my $pad1 = $c1->cd(1)->as('TPad'); # FIXME workaround for enum bug
$pad1->SetGrid();
my $flaplace = TF1->new("flaplace", "TMath::LaplaceDist(x, [0], [1])", -10, 10);
$flaplace->SetParameters(0, 1);

my $f1 = $flaplace->DrawCopy()->as('TF1'); # FIXME workaround for enum bug;
$f1->SetLineColor(kRed);
$f1->SetLineWidth(1);
$flaplace->SetParameters(0, 2);

my $f2 = $flaplace->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug
$f2->SetLineColor(kGreen);
$f2->SetLineWidth(1);
$flaplace->SetParameters(2, 1);

my $f3 = $flaplace->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug;
$f3->SetLineColor(kBlue);
$f3->SetLineWidth(1);
$flaplace->SetParameters(2, 2);

my $f4 = $flaplace->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug;
...
my $pad2 = $c1->cd(2)->as('TPad'); # FIXME workaround for enum bug
$pad2->SetGrid();
my $flaplacei = TF1->new("flaplacei", "TMath::LaplaceDistI(x, [0], [1])", -10, 10);
$flaplacei->SetParameters(0, 1);
my $g1 = $flaplacei->DrawCopy()->as('TF1'); # FIXME workaround for enum bug
$g1->SetLineColor(kRed);
$g1->SetLineWidth(1);
$flaplacei->SetParameters(0, 2);

my $g2 = $flaplacei->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug;
$g2->SetLineColor(kGreen);
$g2->SetLineWidth(1);
$flaplacei->SetParameters(2, 1);

my $g3 = $flaplacei->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug;
$g3->SetLineColor(kBlue);
$g3->SetLineWidth(1);
$flaplacei->SetParameters(2, 2);

my $g4 = $flaplacei->DrawCopy("same")->as('TF1'); # FIXME workaround for enum bug;
